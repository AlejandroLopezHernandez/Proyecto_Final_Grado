// moduloProducto.js
// ==================
// Versión adaptada y corregida para manejo de persistencia sin recargas indebidas
// Asegúrate de cargarlo con <script src=".../moduloProducto.js"></script> en la plantilla.
...
function inicializarModuloProducto(mesa) {
    ...
    if (!panelComanda) {
        console.warn('moduloProducto: panelComanda no encontrado, abortando render de comanda.');
    }
    ...
    if (!panelTipos || !panelCategorias || !panelProductos || !panelOpciones) {
        console.warn('moduloProducto: faltan elementos de navegación de productos, listeners de producto no se asignarán.');
    }
    ...
    function guardarTodasComandas() {
        ...
        try {
            localStorage.setItem('comandasPorMesa', JSON.stringify(estadoApp.comandasPorMesa));
        } catch (e) {
    ...
    if (mesa && mesa.id != null) {
        ...
        if (tituloMesaEl) {
            tituloMesaEl.textContent = `Mesa: ${mesa.nombre || mesa.id}`;
        }
        // Cargar comanda previa
        cargarComandaDeLocalStorage(mesa.id);
    } else {
        // Sin mesa al inicio, se espera seleccionar luego
        console.log('inicializarModuloProducto sin mesa; se espera seleccionar mesa luego.');
    }
    ...
    function getImagePath(nombre, categoria, tipoEspecial = null) {
        ...
        if (['arroces_pastas', 'mar', 'entre_panes', 'vegetariano', 'vegano', 'carnes', 'entrantes', 'ensaladas', 'postres']
    ...
    if (panelTipos) {
        panelTipos.addEventListener('click', async (e) => {
            ...
            if (!btn) return;
            // Reset UI niveles 2,3,4
            document.querySelectorAll('#panel-nivel-1 .categoria-btn').forEach(b => b.classList.remove('active'));
            ...
            try {
                const response = await fetch(`/${tipo === 'bebida' ? 'bebida/tipos' : tipo}`);
                if (tipo === 'bebida') {
                    const result = await response.json();
                    if (panelCategorias) {
                        panelCategorias.innerHTML = result.map(cat => `
                ...
                if (panelCategorias) panelCategorias.innerHTML = '<p class="error">Error al cargar categorías</p>';
            }
        });
    }
    ...
    if (panelCategorias) {
        panelCategorias.addEventListener('click', async (e) => {
            const btn = e.target.closest('.categoria-btn');
            if (!btn) return;
            document.querySelectorAll('#panel-nivel-2 .categoria-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            if (panelProductos) panelProductos.innerHTML = '';
            if (panelOpciones) panelOpciones.innerHTML = '';
            ...
            const tipoSeleccionado = document.querySelector('#panel-nivel-1 .categoria-btn.active')?.dataset.value || 'comida';

            try {
                if (tipoSeleccionado === 'bebida' && ['cerveza', 'destilados'].includes(categoria.toLowerCase())) {
                    const response = await fetch(`/bebida/${categoria}/estilos`);
                    const estilos = await response.json();
                    if (panelProductos) {
                        panelProductos.innerHTML = estilos.map(estilo => `
                            <button class="producto-btn estilo-btn with-image"
                                    style="background-image: url('${getImagePath(estilo, categoria, 'estilo')}')"
                                    data-tipo="bebida"
                                    data-categoria="${categoria}"
                                    data-estilo="${estilo}">
                                <span class="btn-text">${estilo.toUpperCase().replace(/([a-z])\s/g, '$1<br>')}</span>
                            </button>`).join('');
                        panelProductos.querySelectorAll('.estilo-btn').forEach(el => {
                            el.addEventListener('click', function () {
                                panelProductos.querySelectorAll('.estilo-btn').forEach(b => b.classList.remove('active'));
                    ...
                    if (panelProductos) {
                        panelProductos.innerHTML = bebidas.map(bebida => `
                            <button class="producto-btn with-image"
                                    style="background-image: url('${getImagePath(bebida.nombre, categoria)}')"
                                    data-tipo="bebida"
                                    data-id="${bebida.id}"
                                ...
                                <span class="btn-text">
                                    ${bebida.nombre.toUpperCase().replace(/\s+/g, '<br>')}<br>
                                    ${bebida.formato ? bebida.formato.toUpperCase() : ''}
                                </span>
                            </button>`).join('');
                    }
                } else {
                    const response = await fetch(`/${tipoSeleccionado}/categoria/${categoria}`);
                    let data;
                    try { data = await response.json(); } catch { data = null; }
                    if (Array.isArray(data)) {
                        if (panelProductos) {
                            panelProductos.innerHTML = data.map(producto => `
                                <button class="producto-btn with-image"
                                        style="background-image: url('${getImagePath(producto.nombre, categoria)}')"
                                        data-tipo="${tipoSeleccionado}"
                                        data-id="${producto.id}"
                                        data-nombre="${producto.nombre}"
                                        data-precio="${producto.pvp}">
                                    <span class="btn-text">${producto.nombre.toUpperCase().replace(/\s+/g, '<br>')}</span>
                                </button>`).join('');
                        }
                    } else {
                        const text = await response.text();
                ...
                if (panelProductos) panelProductos.innerHTML = '<p class="error">Error al cargar productos</p>';
            }
        });
    }
    ...
    if (panelProductos && panelOpciones) {
        panelProductos.addEventListener('click', async (e) => {
            ...
            if (tipo === 'bebida' && estilo) {
                try {
                    const response = await fetch(`/bebida/${categoria}/${estilo}/registros`);
                    const bebidas = await response.json();
                    ...
                    panelOpciones.querySelectorAll('.producto-btn').forEach(el => {
                        el.addEventListener('click', function () {
                            const nombreCompleto = `${this.dataset.nombre} ${this.dataset.formato || ''}`.trim();
                            estadoApp.productoSeleccionado = { nombre: nombreCompleto, precio: parseFloat(this.dataset.precio) || 0, extras: [] };
                            agregarProducto(nombreCompleto, parseFloat(this.dataset.precio) || 0, []);
                            this.classList.add('active');
                        });
                    });
                    ...
                    panelOpciones.innerHTML = '<p class="error">Error al cargar opciones</p>';
                }
                return;
            }
            ...
            estadoApp.productoSeleccionado = {
                nombre: tipo === 'bebida' ? `${nombre} ${formato || ''}`.trim() : nombre,
                precio: precio,
                id: id,
                tipo: tipo,
                extras: []
    ...
    function agregarProducto(nombre, precio, extras = []) {
        ...
        const existente = estadoApp.comandasPorMesa[key].find(linea =>
            linea.nombre === nombre &&
        ...
        renderizarComanda();
        console.log(`agregarProducto llamado con mesaActiva= ${estadoApp.mesaActiva}  nombre= ${nombre}  extras=`, extras);
    ...
    function renderizarComanda() {
        ...
        comanda.forEach((linea, idx) => {
            ...
            detalleSpan.textContent = `${cantidad} x ${linea.precio.toFixed(2)} € = ${subtotal.toFixed(2)} €`;
        ...
        // Total general
        const totalGeneral = comanda.reduce((sum, linea) =>
            sum + (linea.precio * (linea.cantidad || 1)), 0);
    ...
    // Cambia de mesa activa dentro del módulo de producto
    function cambiarMesa(mesaId, nombreMesa) {
        ...
        const tituloMesaEl = document.getElementById("nombre-mesa-activa");
        if (tituloMesaEl) {
            tituloMesaEl.textContent = nombreMesa ? `Mesa: ${nombreMesa}` : `Mesa: ${mesaId}`;
        }
        cargarComandaDeLocalStorage(estadoApp.mesaActiva);
    }
    window.cambiarMesa = cambiarMesa;

    // Imprime ticket: genera canvas → ventana nueva + print → guarda en BD → limpia localStorage y redirige
    async function imprimirTicket() {
        ...
        const total = comanda.reduce((sum, item) =>
        ...
        const altura = padding * 2 + headerLines * lineHeight
            + comanda.length * lineHeight + extraLines * lineHeight + lineHeight;
        ...
        comanda.forEach(item => {
            ...
            y += lineHeight;
            ctx.fillText(`${cantidad} x ${item.precio.toFixed(2)} = ${subtotal} €`, padding + 10, y);
            y += lineHeight;
            if (Array.isArray(item.extras) && item.extras.length) {
                item.extras.forEach(extra => {
                    ctx.fillText(`+ ${extra}`, padding + 20, y);
                    y += lineHeight;
                });
            }
        });
        ...
        window.location.href = '/panel'; // o ajusta a la ruta de tu panel principal
    }
